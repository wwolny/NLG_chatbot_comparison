...
class RNNModel(BaseModel):
    def __init__(
        ...
        if path2model is not None:
            ...
            self.one_step_model = OneStepModel(
                self.model, self.chars_from_ids, self.ids_from_chars
            )
    ...
    def train(self, output_path: str, train_config: TrainConfig):
        ...
        sequences = ids_dataset.batch(
            train_config.seq_length + 1, drop_remainder=True
        )
        dataset = sequences.map(self.split_input_target)

        checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
            filepath=output_path, save_weights_only=True
        )

        dataset = (
            dataset.shuffle(train_config.buffer_size)
